...
static uint8_t But2Deb; //

if(BUTTON1_PRESS) //если кнопка 1 нажата
	{
	But1Deb=0; //сброс антидребезга на отпускание

	if(++But1Cnt>But1Time) //верхняя граница счетчика "But1Cnt" - время удержания кнопки
		{
		But1Cnt=0; //сброс счетчика
	...
	{
	if(++But1Deb > BUTT_TIME_DEB) //антидребезг на отпускание кнопки
		{
		But1Deb=0;
		//нижняя граница счетчика "But1Cnt" - пауза антидребезга на нажатие кнопки
...
PORTD=0b01000100; //6 вкл. питание LCD //2 подтягивающий резистор на вход импульсов

TCCR0B=0b00000101; //2-0 предделитель  (таймер ~100Гц для фоновых операций)
TCNT0=0x64;

TCCR1B=0b00000101; //2-0 предделитель  (таймер 1Гц для измерениий)
TCNT1=TMR1_PRELOAD;

TCCR2B=0b00000101; //101 Fcpu/128=125000 //100 Fcpu/64=250000  (таймер для пищалки)
OCR2A=BUZZ_2K;
...
#ifndef TEST_BUILD
uint16_t cnt=1200; //1200*1мс=1,2с длительность накачки преобразователя при включении
#else
uint16_t cnt=0;
#endif

while(cnt || BUTTON_PRESS) //преднакачка 1,2с или пока удерживается любая кнопка
	{
	if(cnt) cnt--;
	hvgen_impuls();
	_delay_ms(1);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void system_shutdown(void) //выключение
{
...
PCMSK0=0b00011000; //изменение состояния пинов 3 и 4 (кнопки) будет вызывать PCINT0
PCICR=0b00000001; //разрешаем прерывание PCINT0

SMCR=0b00000101; //выбираем режим сна и разрешаем МК спать
sleep_cpu();
//сон
...
else { sei(); system_shutdown(); } //иначе выключаемся
}
...
while(1)
	...
	switch(get_button()) //действие по нажатой кнопке
		...
		case 4: //удерживается кнопка 2
			lcd_clear();
			strcpy_P(StrBuff,(PGM_P)pgm_read_word(&MsgStr[7])); //выводим сообщение о выключении
			lcd_string(4,3,StrBuff);
			draw_char_line(2,161);
			draw_char_line(6,161);
			_delay_ms(2000);
			short_beep(50,BUZZ_1K);
			system_shutdown(); //выключение прибора
			refrTmr=0;
			break;
		}
	}
